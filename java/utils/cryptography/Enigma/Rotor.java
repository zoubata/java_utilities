...
public class Rotor {
	...
	/**
	 * Assuming that P is an integer in the range 0..25, returns the corresponding
	 * upper-case letter in the range A..Z.
	 */
	...
	/**
	 * Return my current rotational position as an integer between 0 and 25
	...
	/**
	 * Return the conversion of P (an integer in the range 0..25) according to my
	 * permutation.
	 */
	int convertForward(int p) {
		return ((cipher[((p + position) % Machine.CharSetSize + Machine.CharSetSize) % Machine.CharSetSize] - position)
				% Machine.CharSetSize + Machine.CharSetSize) % Machine.CharSetSize;
	}

	/**
	 * Return the conversion of E (an integer in the range 0..25) according to the
	 * inverse of my permutation.
	 */
	int convertBackward(int e) {
		return ((bcipher[((e + position) % Machine.CharSetSize + Machine.CharSetSize) % Machine.CharSetSize] - position)
				% Machine.CharSetSize + Machine.CharSetSize) % Machine.CharSetSize;
	}

	/**
	 * Returns true iff I am positioned to allow the rotor to my left to advance.
	 */
	boolean atNotch() {
	...
	/**
	 * My current position (index 0..25, with 0 indicating that 'A' is showing).
	 */
