...
public class PassWordChecker {
	...
	/**
	 * password checker : - basic check of password - don't contain a first name of
	 * more than 3 character - don't contain a last name of more than 3 character to
	 * return true example "P!erre=3.141592"
	 */
	...
	/**
	 * password checker : - strong check of password - don't contain a first name
	 * even if you replace some letter by special letter. - don't contain a last
	 * name even if you replace some letter by special letter. to return true
	...
	public static boolean checkVeryStrongPassWord(String passordl) {
		...
		passordl = passordl.replace('!', 'i').replace('@', 'a').replace('а', 'a');
		passordl = passordl.replace('&', 'e').replace('й', 'e').replace('и', 'e');
		passordl = passordl.replace('к', 'e').replace('0', 'o').replace('щ', 'u');
		passordl = passordl.replace('µ', 'u').replace('$', 's').replace('%', 'x');
		passordl = passordl.replace('|', 'i').replace('з', 'c').replace('Ј', 'l');
	...
	/**
	 * password checker : it should have - more than 12 character - a letter lower
	 * case - a letter upper case - a number - a special character :
	 * &й"'(-и_за)=}]@^\`|[{#& - more than 7 different character to return true
	...
	public static boolean checkPassWord(String passord) {
		...
		if (!passord.matches(".*[A-Z]+.*"))// a letter upper case
			return false;
		if (!passord.matches(".*[~<>,;:!?/\\.\\$+-°\\*/§&й\"'\\(-и_за\\)=~#\\{\\[\\|`\\\\^@\\]\\}]+.*"))// a special
																										// char
			return false;
