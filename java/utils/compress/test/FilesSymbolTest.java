...
public class FilesSymbolTest {
	...
	public void testSym_LZs() {
		...
		assertEquals("1" + "10000001" + "00", s.getCode().toRaw());
	...
	public void testFilesSymbolSetOfFile() {
		/*
		 * File f1 = new File("res/test/smallfile.txt"); File f2 = new
		 * File("res/test/medium.txt"); Set<File> fset=new HashSet<File>();
		 * fset.add(f1); fset.add(f2);
		 * 
		 * FilesSymbol fs=new FilesSymbol(fset,f2.getAbsoluteFile().getParent());
		 * FilesSymbol fs1=new FilesSymbol(f1); FilesSymbol fs2=new FilesSymbol(f2);
		 * 
		 * assertEquals(fs.toSymbol().size(),fs2.toSymbol().size()+fs1.toSymbol().size()
		 * );
		 * 
		 * File fd = new File("res/test/ref"); FilesSymbol fds=new FilesSymbol(fd);
	...
	public void testFilesSymbolFile() {
		...
		assertEquals(lsin.size(),
				(f.length() + 2 + f.getAbsolutePath().length() - f.getAbsoluteFile().getParent().length() + 2));
		assertTrue(lsin.size() >= (f.length() + 2 + f.getAbsolutePath().length()
				- f.getAbsoluteFile().getParent().length() + 2));
	...
	public void testToSymbol() {
		...
		assertEquals(JavaUtils.read("res\\test\\ref\\smallDir" + "\\smallfile.txt"),
				JavaUtils.read("res\\result.test\\tmp\\testToSymbol\\" + "smallfile.txt"));
		assertEquals(JavaUtils.read("res\\test\\ref\\smallDir" + "\\int.txt"),
				JavaUtils.read("res\\result.test\\tmp\\testToSymbol\\" + "int.txt"));
