...
public class Node2<T extends Comparable<? super T>, V> {
	...
	public String toString() {
		...
		/*
		 * // return symbol+"("+childrens.size()+","+length()+")"; StringBuffer s=new
		 * StringBuffer(); //childrens=SortMapBykey(childrens);
		 * //childrens.remove(null); if(childrens==null || childrens.values().size()==0)
		 * { if (symbol!=null) s.append("\r\n"+symbol); else s.append(""); } else for(T
		 * ee:(getChildrens().keySet())) { Node2<T,V> e=getChildrens().get(ee); try { if
		 * (e.getSymbol()!=Symbol.findId('\n') && e.getSymbol()!=Symbol.findId('\\') &&
		 * symbol!=Symbol.findId('$')) s.append(e.toString().replaceAll("\r\n",
		 * "\r\n"+((symbol==null)?"":(symbol.toString())))); }
		 * catch(java.lang.IllegalArgumentException ex) {
		 * System.err.println("for symbol '"+e.getSymbol().toString()+"' or '"+this.
		 * getSymbol()+"'"); ex.printStackTrace(); }; } return s.toString();
		 */
	}
	...
	/*
	 * abcdabcf abcf bcda cda da A {a} ababababc abc ababc babc babababc bc c
	 */

	/*
	 * rootnode=getRootNode(); List<Node2<T,V>> l=getNodes(rootnode.getSymbol())
	 * l=l.reverse();
	 * 
	 * for(i=0,i<len,i++){ tree.addtree(l);l.remove(0);}
	 */
	public V getIndex() {
		return index;
	}
	...
	public String toGraphViz(Tree tree) {

		String s = "";
		if (tree.cached.contains(this))
			return "";
		/*
		 * if(getRootNode().getChildrens().values().contains(this)) return "";
		 */
		for (T c : getChildrens().keySet())
			if (!tree.cached.contains(getChildren(c))) {
				s += "\"" + getSymbol() + "_" + getIndex() + "_" + hashCode() + "\" -> \"" + getChildren(c).getSymbol()
						+ "_" + getChildren(c).getIndex() + "_" + getChildren(c).hashCode() + "\"\r\n";
			}
		tree.cached.add(this);
		for (T c : getChildrens().keySet())
			if (!tree.cached.contains(getChildren(c)))
				s += getChildren(c).toGraphViz(tree);
		return s;
