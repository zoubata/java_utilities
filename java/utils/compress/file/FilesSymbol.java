...
public class FilesSymbol {
	...
	/*
	 * /** Split the list of symbol on sub list to optimize the huffman algo on each
	 * list of symbol.
	 * 
	 * It is usefull hen the nature of the sequence of symbol isn'h homogen exemple,
	 * a text file foolowed by a binary file. followed by a csv tabe file. each
	 * sublist of symbol have it own entropy that was significantly different.
	 * 
	 * The order is keeped, to keep integrity of other compression method working at
	 * symbol level. * public static List<List<ISymbol>> split(List<ISymbol> ldec) {
	 * // internal algo on each file analyse frequency and difference, split is
	 * difference is significative.
	 * 
	 * List<List<ISymbol>> ll=new ArrayList<List<ISymbol>> (); int
	 * toIndex=ldec.indexOf(Symbol.EOF); while(toIndex>0) {
	 * 
	 * ll.add( ldec.subList(0, toIndex)); ldec=ldec.subList( toIndex+1,ldec.size());
	 * toIndex=ldec.indexOf(Symbol.EOF); } ll.add(ldec); return ll; }
	 */
	/*
