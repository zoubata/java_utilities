...
public class Ecc {
	...
	/** return the corected word based on eccdata
	 * word max size is 64 byte
	 * */
	public byte[] getdata( byte[] word,byte[] ecc)
	{
		...
		if(ecc.length<8)
		{
		/*if((ecc[0]&0x1)==(ecc2[0]&0x1))
		return null;//more than 1 error
	*/	long lecc=0;
		long lecc2=0;
			...
			112=70=01110000
			     ...
			     * word w0 w1 w2 w3 w4 w5 w6 w7 w8 w9 w10 w11 w12 w13 w14 w15 w16 ......
			  	 X  p   *  *  *  *  *  *  *  *  *  *  *   *   *   *   *   *   *
			  	 X  odd    *     *     *     *     *      *       *       *    
			  	 X  p2        *  *        *  *        *   *           *   *    
			  	 *  p3              *  *  *  *                *   *   *   *  
	 ...
	 *  p3              *  *  *  *                *   *   *   *    
	 *  ecc.lec
	 * return ecc[0..n]=parity:odd:p2....
	 * the size of word ecc is automatic, 
	 * if word size is 2^n bits, ecc is n+2 bit size.
	 * */
...
//			System.out.println("table3["+i+"]="+String.format("%2x", FAST_parity[i]));
	...
	//	System.out.println((short)(ecc[0]&0xff)+"\t"+"0123");
		/*
		for(int j=4;j<lecc;j++)
		for(int i=0;i<lw;i+=1<<(j-3))
