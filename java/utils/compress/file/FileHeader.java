...
public class FileHeader {
	...
	// when encryption is enable the encryption is done block by block, to allow to
	// decrypt just a block.
	...
	// Serpent
	// Square
	boolean SeedUsed = true;// used a seed to protect password from analytic attack by inserting on hash a
							// random number known that will be different on each use of password.
	int seed;// germ used to generate the keys this is a random number.
	boolean KeyStretching = true;// The key is generated from a user-supplied passphrase using an algorithm based
									// on the SHA-256 hash function. The SHA-256 is executed 218 (262144) times,[4]
									// which causes a significant delay on slow PCs before compression or extraction
									// starts. This technique is called key stretching and is used to make a
									// brute-force search for the passphrase more difficult. Current GPU-based, and
									// custom hardware attacks limit the effectiveness of this particular method of
									// key stretching,[5] so it is still important to choose a strong password.
	// SHA256 x2^12
	// CRC x12
	boolean KeyMultiplication = true;// generate several key on the file.
	boolean HeaderEncrypted = false;// if true also the header from this point is encrypted like a block.also the
									// tearoff is encrypted on this case, so intigrity of archived can't be check
	...
	// file's data are identify by a block id[32] and a bit index[32] inside the
	// block.
	// block is located by a bit index[64] inside the overall file. last block is a
	// 0 length block, so length is index(b+1)-index(b)
	// each block are encrypted independently with a different key.
	// block max size is 2^64 bit(including header/tearoff).
	// fat can include dummy block like an old fat block, a stickbit dummy identify
	// them.

	boolean tmp = false;
	boolean CheckSumByBlock = false;
	byte CheckSumAlgoByBlock;// [7:2]algo; [1:0] version
								// //MD5/SHA1/SHA256/SHA512/CRC32/CRC64/Adler32/RIPEMD-160/XOR/BLAKE2(https://en.wikipedia.org/wiki/BLAKE_(hash_function)#BLAKE2)/Authenticode
								// Signature(https://en.wikipedia.org/wiki/Code_signing)
	boolean GlobalCheckSum = false;
	byte GlobalCheckSumAlgo;// [7:2]algo; [1:0] version
							// //MD5/SHA1/SHA256/SHA512/CRC16/CRC32/CRC64/Adler32/RIPEMD-160/XOR/BLAKE2(https://en.wikipedia.org/wiki/BLAKE_(hash_function)#BLAKE2)/Authenticode
							// Signature(https://en.wikipedia.org/wiki/Code_signing)
	int globalCrc = 0;
