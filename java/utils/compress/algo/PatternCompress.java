...
public class PatternCompress {
	...
	public static void main(String[] args) {
		...
		List<ISymbol> ls;
		FFT(ls=Symbol.from(JavaUtils.read("C:\\Temp\\CharacArmIdFreshDieVddSearch_tg.csv")));
		JavaUtils.saveAs("C:\\temp\\t.tmp",Symbol.toString(Symbol.join(Symbol.transpose(Symbol.Split(ls,Symbol.findId('\n')),Symbol.Empty))));
		...
		System.out.println("lst size" + lst.size() + "/" + lste.size() + "rle size " + lse.size() + "rle size #"
				+ "  size=" + count + " : rle=" + rle + ": newsize~=" + (count - rle + 3 * m.keySet().size()));
...
*/
	...
	static public Map<ISymbol, Map<Long, Long>> FreqDist(List<ISymbol> ls) {
		...
		/*
		 * for(int i=0;i<nb;i++) { index[i]=null; m.put(Symbol.findId(i), new
		 * HashMap<Long, Long>()); }
		 */
		long count = 0;
		for (ISymbol s : ls) {
	...
	/**
	 * regarding the concept it is a little bit like fft for mp3, but we will speak
	 * about periodicity instead of frequency, occurency instead of amplitude the
	 * periodicity is the distance between 2 times that a symbol happen. the
	 * occurence is the number of times where this periodicity happens. * the goal
	 * is to catch the periodicity that happen the more often for all symbol to find
	 * the structure of the file. after we extract a pattern with fix field, and
	 * variable area.
	 * 
	 **/
	static public void FFT(List<ISymbol> ls) {
		int nb = Symbol.getNbSymbol();
		/*
		 * Long index[]=new Long[nb]; Map<ISymbol, Map<Long, Long>> m=new
		 * HashMap<ISymbol, Map<Long, Long>>(); for(int i=0;i<nb;i++) { index[i]=null;
		 * m.put(Symbol.findId(i), new HashMap<Long, Long>()); } long count=0;
		 * for(ISymbol s:ls) { if(index[(int)s.getId()]!=null) { Map<Long, Long> mm =
		 * m.get(s); long dist=count-index[(int)s.getId()];
		 * 
		 * Long lc=mm.get(dist); if (lc==null) lc=0l; else lc++; mm.put(dist, lc); }
		 * index[(int)s.getId()]=count; count++; }
		 */
		Map<ISymbol, Map<Long, Long>> m = FreqDist(ls);
		/*
		 * double sum=0.0; Map<Long, Long> mr =new HashMap<Long, Long>(); for(int
		 * i=0;i<nb;i++) { Map<Long, Long> mm = m.get(Symbol.findId(i));
		 * m.put(Symbol.findId(i), mm=JavaUtils.SortMapByValue(mm)); double
		 * e=HuffmanCode.getEntropie(mm);
		 * System.out.println(Symbol.findId(i)+":"+e+":"+mm); sum+=e*m.keySet().size();
		 * for(Long k:mm.keySet()) { Long lc=mm.get(k); Long lr=mr.get(k); if(lc==null)
		 * lc=0L; if(lr==null) lr=0L; lr+=lc; mr.put(k, lr); } }
		 * mr=JavaUtils.SortMapByValue(mr); System.out.println("e="+sum+":"+mr);
		 */
		Map<Long, Long> mr = FreqDistSum(m);
		Freq(m);
	...
	/**
	 * from the freq of distance per symbol, it build a summary of frequency of
	 * distance overall symbol.
	 * 
	 */
	...
	private static Map<Long, Long> FreqDistSum(Map<ISymbol, Map<Long, Long>> m) {
		...
		for (int i = 0; i < nb; i++) {
			...
			if (mm != null) {
				...
				System.out.println(
						Symbol.findId(i) + ":" + e + ":" + mm.keySet().size() + "/" + localcountSym + ":" + mm);
	...
	public static void main3(String[] args) {
		...
		// map(line,len)
		Map<List<ISymbol>, Long> m = lss.stream()
				.collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
		m = JavaUtils.SortMapByValue(m);

		// map(len,count)
		Map<List<ISymbol>, Integer> n = m.keySet().stream().collect(Collectors.toMap(Function.identity(), List::size));
		n = JavaUtils.SortMapByValue(n);
		Map<Integer, Long> o = n.values().stream()
				.collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
		...
		o = o.entrySet().stream().filter(x -> x.getValue() >= 3)
				.collect(Collectors.toMap(x -> x.getKey(), x -> x.getValue()));
		{
			final Set<Integer> lo = o.keySet();
			n = n.entrySet().stream().filter(x -> lo.contains(x.getValue()))
					.collect(Collectors.toMap(x -> x.getKey(), x -> x.getValue()));

			for (Integer l : lo) {
				List<List<ISymbol>> n1 = n.entrySet().stream().filter(x -> l.equals(x.getValue())).map(x -> x.getKey())
						.collect(Collectors.toList());
				Stream<List<Map<ISymbol, Integer>>> n2 = n1.stream().map(x -> x.stream().map(y -> {
					Map<ISymbol, Integer> t = new HashMap<ISymbol, Integer>();
					t.put(y, (Integer) 1);
					return t;
				}).collect(Collectors.toList()));
				 ...
				 * Map<ISymbol,Integer> m1=result.get(i); Map<ISymbol,Integer>
				 * m2=article.get(i); m2.forEach((k, v) -> m1.merge(k, v, (v1, v2) ->((new
				 * Integer( v1 + v2)))));
				 * 
		...
		/*
		 * 
		 * n.filter(value=42).findstructure List<histo(ysm,count))=n.filter(value=42).do
		...
		HuffmanNode rootnode = HuffmanCode.buildTrie(mfreq);
		...
		JavaUtils.saveAs("res/result.test/test/small_ref/pat.pietxt",
				lsc.stream().map(Object::toString).collect(Collectors.joining(",")));
	...
	public List<ISymbol> compress(List<ISymbol> ls) {
		...
		/*
		 * tree=new Tree(); currentLeaf=tree.getRoot(); index=0L; for(ISymbol s:ls) {
		 * List<ISymbol> sc=processcompress(s); if (sc!=null) lsc.addAll(sc);
		 * //1tree.add(0L,index,ls); //tree.add(index-1, s); } List<ISymbol>
	...
	public List<ISymbol> uncompress(List<ISymbol> ls) {
		...
		/*
		 * index=0L; n1=n2=null; uncompressedflux=new ArrayList();
		 * 
		 * for(ISymbol s:ls) { List<ISymbol> sc=processUncompress(s);
		 * 
		 * }
		 */
		return lsd;
