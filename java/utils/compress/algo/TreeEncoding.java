 ...
 * this algo use tree to define a dictionnary.
 */
	...
	/*	int i=readDictionary(lenc);
		...
		*/
		for(ISymbol s:lenc)
		{
			...
			if(cs!=null && Symbol.NewWord.equals(cs.getS0()))
			{
				...
				m.put(
						new CompositeSymbols(Symbol.Word, new Number(indexdico)),
						ws);
				//ldico.add(ws);
				indexdico++;
		...
		for( i=0;i<ldec.size();)
		{
			...
			else
			{
					...
					if (good.getSymPacked()==null)
					{
						...
						lse.add(new CompositeSymbols(Symbol.NewWord, new Number(good.getDeep())));
						good.setSymPacked(new CompositeSymbols(Symbol.Word, new Number(indexdico)));
						ldico.add(good);
						indexdico++;
					}
					else
