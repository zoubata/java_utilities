...
public class MACD implements Indicator {
    ...
    public MACD(Stock action, int shortPeriod, int longPeriod, int signalPeriod) {
        this.shortEMA = new EMA(action, shortPeriod, true); //true, because history is needed in MACD calculations.
        this.longEMA = new EMA(action, longPeriod, true); //true for the same reasons.
        this.period = signalPeriod;
        this.multiplier = 2.0 / (double) (signalPeriod + 1);
        this.periodDifference = longPeriod - shortPeriod;
        explanation = "";
        init(action); //initializing the calculations to get current MACD and signal line.
    }
