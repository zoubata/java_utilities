...
public class Knight extends Part  implements IPart {

	public Knight(char myteam) {
		team=myteam;
		partid='k';
	}
	@Override
	public boolean isMoveAllow(int x1, int y1, int x2, int y2) {
		if (getBoard()==null)
			return false;
		boolean b=(Math.abs(x1-x2)==2) && (Math.abs(y1-y2)==1);
		b=b||(Math.abs(x1-x2)==1) && (Math.abs(y1-y2)==2);
